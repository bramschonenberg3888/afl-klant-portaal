...
// Mock modules that use server-side environment variables before importing the router
vi.mock('@/lib/auth', () => ({
  auth: vi.fn(),
}));
...
vi.mock('@/lib/scraper/ingest', () => ({
  ingestUrl: vi.fn(),
  ingestMultipleUrls: vi.fn(),
}));

vi.mock('@/lib/scraper/firecrawl', () => ({
  scrapeUrl: vi.fn(),
  crawlSite: vi.fn(),
}));

vi.mock('@/lib/vector/pgvector', () => ({
